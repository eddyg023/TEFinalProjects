    ...
    private void initialize() {
        ...
        try {
            ...
            for (String memberLine : memberLines) {
                String[] fields = memberLine.split(FIELD_DELIMITER);
                String firstName = fields[0];
                String lastName = fields[1];
                String dataPath = FILE_BASE_PATH + fields[2];

                Member currentMember = new Member(firstName, lastName);
                List<CatalogItem> currentMemberItems = new ArrayList<>();

                List<String> itemLines = FileStorageService.readContentsOfFile(dataPath);
                for (String itemLine : itemLines) {
                    String[] itemFields = itemLine.split(FIELD_DELIMITER);

                    if (itemFields.length != 4) {
                        continue;
                    }

                    String itemType = itemFields[0];

                    if(itemType.equals("book")) {
                        String title = itemFields[1];
                        String author = itemFields[2];
                        LocalDate publishDate = LocalDate.parse(itemFields[3]);

                        Book book = new Book(title, author, publishDate);
                        book.registerItem();
                        currentMemberItems.add(book);

                    } else if (itemType.equals("movie")) {
                        String name = itemFields[1];
                        String director = itemFields[2];
                        LocalDate releaseDate = LocalDate.parse(itemFields[3]);

                        Movie movie = new Movie(name, director, releaseDate);
                        movie.registerItem();
                        currentMemberItems.add(movie);

                    } else if (itemType.equals("tool")) {
                        String type = itemFields[1];
                        String manufacturer = itemFields[2];
                        int count = Integer.parseInt(itemFields[3]);

                        Tool tool = new Tool(type, manufacturer, count);
                        tool.registerItem();
                        currentMemberItems.add(tool);
                    }
                }
                catalog.put(currentMember.toString(), currentMemberItems);
            }
